...
-- Goal: Allow leads to Delete Members from Their Project
-- ============================================
...
-- 2. Add policy for leads to delete members in their projects
CREATE POLICY "m_lead_delete" ON public.project_members
  FOR DELETE TO authenticated
  USING (
    is_project_lead(project_id, auth.uid())
  );

-- 3. Add policy for leads to update member permissions
-- The current schema might already allow it if it's "all" but let's be explicit
DROP POLICY IF EXISTS "m_update_lead" ON public.project_members;
CREATE POLICY "m_update_lead" ON public.project_members
