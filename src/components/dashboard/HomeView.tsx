...
const HomeView = ({ user, tasks, onAddTask, onTasksUpdated }: HomeViewProps) => {
    ...
    return (
        <div className="p-4 md:p-8 max-w-7xl mx-auto h-full flex flex-col font-sans gap-8">
            {/* Header Section */}
            <header className="flex flex-col md:flex-row md:items-center justify-between gap-6">
                ...
                <div className="flex items-center gap-3 shrink-0">
                    <div className="px-5 py-3 bg-white dark:bg-zinc-900 border border-zinc-200 dark:border-zinc-800 rounded-2xl shadow-sm">
                        <div className="flex items-center gap-2 mb-1">
                            <CheckCircle2 className="w-3.5 h-3.5 text-emerald-500" />
                            <span className="text-[10px] font-bold text-zinc-500 uppercase tracking-wider">Done</span>
                        </div>
                        <div className="flex items-baseline gap-2">
                            <span className="text-xl font-bold text-zinc-900 dark:text-white">{completedTasks}</span>
                        </div>
                    </div>
                    <div className="px-5 py-3 bg-white dark:bg-zinc-900 border border-zinc-200 dark:border-zinc-800 rounded-2xl shadow-sm">
                        <div className="flex items-center gap-2 mb-1">
                        ...
                        <div className="flex items-baseline gap-2">
                            <span className="text-xl font-bold text-zinc-900 dark:text-white">{overdueTasks}</span>
                        </div>
                    </div>
            ...
            <div className="grid grid-cols-1 lg:grid-cols-3 gap-8 flex-1 min-h-0">
                ...
                <div className="lg:col-span-2 flex flex-col min-h-0">
                    <div className="bg-white dark:bg-zinc-900 border border-zinc-200 dark:border-zinc-800 rounded-[32px] shadow-xl shadow-zinc-200/50 dark:shadow-none overflow-hidden flex flex-col h-full min-h-[500px]">
                        {/* Task Card Header */}
                        <div className="flex items-center justify-between px-6 py-5 border-b border-zinc-100 dark:border-zinc-800/50 sticky top-0 bg-white/80 dark:bg-zinc-900/80 backdrop-blur-md z-10 transition-colors">
                            <div className="flex items-center gap-4">
                                <div className="w-10 h-10 rounded-xl bg-violet-600 flex items-center justify-center text-white font-bold text-sm shadow-xl shadow-violet-500/20">
                                    {user?.full_name?.[0].toUpperCase()}
                                </div>
                                <div>
                                    <h2 className="text-base font-bold text-zinc-900 dark:text-white tracking-tight">Focus</h2>
                                    <p className="text-[11px] text-zinc-500 font-medium uppercase tracking-wider mt-0.5">Today's Missions</p>
                                </div>
                            </div>
                        </div>

                        {/* Task Filter Tabs */}
                        <div className="px-6 py-4 flex items-center justify-between border-b border-zinc-50 dark:border-zinc-800/30">
                            <div className="flex p-1 bg-zinc-100 dark:bg-zinc-800 rounded-xl">
                                {(['upcoming', 'overdue', 'completed'] as const).map(tab => (
                                    <button
                                        key={tab}
                                        onClick={() => setActiveTab(tab)}
                                        className={`px-4 py-1.5 rounded-lg text-[10px] font-bold uppercase tracking-wider transition-all ${activeTab === tab
                            ...
                            <span className="text-[10px] font-bold text-zinc-500 uppercase tracking-wider">{filteredTasks.length} Tasks</span>
                        </div>
                                ...
                                <div className="space-y-2">
                                    {filteredTasks.map((task, index) => (
                                        <motion.div
                                            ...
                                            key={task.id}
                                            className="group flex items-center gap-4 px-4 py-4 bg-zinc-50 dark:bg-zinc-800/30 border border-transparent hover:border-violet-200 dark:hover:border-violet-800/50 rounded-2xl transition-all cursor-pointer"
                                        >
                                            <button
                ...
                <div className="flex flex-col gap-6">
                    {/* Insights Card */}
                    {/* Goal Card */}
                    <div className="p-6 bg-gradient-to-br from-violet-600 to-indigo-700 rounded-[32px] text-white shadow-xl shadow-violet-500/20 relative overflow-hidden group">
                    ...
                    {/* STREAK WIDGET - THE "OUT OF THE BOX" IDEA */}
                    <div className="p-6 bg-white dark:bg-zinc-900 border border-zinc-200 dark:border-zinc-800 rounded-[32px] shadow-lg relative overflow-hidden flex flex-col items-center">
                        <div className="flex items-center justify-center relative mb-4">
                            {/* Streak Glow Effect */}
                            <div className="absolute inset-0 bg-orange-500/20 blur-2xl rounded-full" />
                            <div className="relative w-16 h-16 bg-gradient-to-br from-orange-400 to-rose-600 rounded-3xl flex items-center justify-center shadow-xl shadow-orange-500/30 transform rotate-3 hover:rotate-0 transition-transform cursor-pointer">
                                <Flame className="w-8 h-8 text-white animate-pulse" fill="white" />
                            </div>
                        </div>
                        ...
                        <div className="w-full mt-6 flex justify-between px-2">
                    ...
                    {/* Motivation Block */}
                    <div className="p-5 flex items-center gap-4 bg-zinc-50 dark:bg-zinc-900/50 border border-zinc-100 dark:border-zinc-800 rounded-[28px] group hover:border-violet-200 dark:hover:border-violet-800 transition-colors">
                        <div className="w-10 h-10 rounded-full bg-white dark:bg-zinc-800 flex items-center justify-center shadow-sm group-hover:rotate-12 transition-transform">
                            <span className="text-lg">✨</span>
                        </div>
                        <div className="flex-1">
                            <p className="text-[11px] font-medium text-zinc-500 uppercase tracking-wider leading-none mb-1">Today's Wisdom</p>
                            <p className="text-[12px] font-bold text-zinc-700 dark:text-zinc-300 leading-snug">"The secret of getting ahead is getting started."</p>
                        </div>
