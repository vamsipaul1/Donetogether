        ...
        <Dialog open={isOpen} onOpenChange={onClose}>
            <DialogContent className="sm:max-w-lg bg-white dark:bg-[#1e1f21] font-sans border-zinc-200 dark:border-[#3d3e40] p-0 overflow-hidden rounded-[32px]">
                <div className="bg-zinc-50 dark:bg-zinc-900 p-5 border-b border-zinc-100 dark:border-zinc-800 flex items-center gap-4">
                    ...
                    <div>
                        <DialogTitle className="text-[18px] font-black tracking-tight text-zinc-900 dark:text-white">Edit Project Details</DialogTitle>
                        <p className="text-[11px] font-black tracking-wide text-zinc-600 mt-0.5">project changes</p>
                    </div>
                </div>

                <div className="p-5 space-y-4">
                    <div className="space-y-1.5">
                        <label className="text-[11px] font-black uppercase tracking-widest text-zinc-500 ml-1">Project Name </label>
                        <Input
                            value={title}
                    ...
                    <div className="space-y-1.5">
                        <label className="text-[11px] font-black uppercase tracking-widest text-zinc-500 ml-1">Team Name (Squad Identity)</label>
                        <Input
                            value={teamName}
                    ...
                    <div className="space-y-1.5">
                        <label className="text-[11px] font-black uppercase tracking-widest text-zinc-500 ml-1">Project Description</label>
                        <Textarea
                            value={goal}
                    ...
                    {members.length > 0 && (
                        <div className="space-y-1.5">
                            <label className="text-[11px] font-black uppercase tracking-widest text-zinc-500 ml-1">Project Members</label>
                            <div className="bg-zinc-50 dark:bg-zinc-900 border border-zinc-200 dark:border-zinc-800 rounded-xl overflow-hidden max-h-40 overflow-y-auto">
                                {members.map((member: any) => (
                                    <div key={member.id} className="flex items-center gap-3 p-2 border-b border-zinc-100 dark:border-zinc-800 last:border-0 hover:bg-zinc-100 dark:hover:bg-zinc-800/50 transition-colors">
                                        ...
                                        <div className="text-[10px] font-bold uppercase tracking-wider text-zinc-500 bg-zinc-100 dark:bg-zinc-800 px-2 py-1 rounded-md">
                ...
                <div className="p-4 bg-zinc-50 dark:bg-zinc-900/50 border-t border-zinc-100 dark:border-zinc-800 flex justify-end gap-3">
                    <Button variant="ghost" onClick={onClose} className="rounded-xl h-9 px-6 text-[11px] font-black uppercase tracking-wider text-zinc-500 hover:text-zinc-900 dark:hover:text-white hover:bg-zinc-200/50 dark:hover:bg-zinc-800">
                        <X className="w-3.5 h-3.5 mr-2" /> cancel
                    </Button>
                    <Button onClick={handleUpdate} disabled={loading} className="bg-emerald-500 hover:bg-emerald-600 text-white font-black text-[11px] uppercase tracking-wider rounded-xl px-5 h-9 shadow-lg shadow-emerald-500/20">
